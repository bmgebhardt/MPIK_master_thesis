...
\caption[Multi p.e. persistence plot]{Persistence figure of the shaped pulse after adapting the electronics to a preferable pulse shape. In the center, a row of pulses with rising amplitudes is visible. These are the multiple N p.e. pulses, differing by $\sim$~5mV. The purple area off-center is due to delayed cross-talk and after-pulsing. Image credit \cite{Initial_testing_at_MPIK_wb_July_18_2016}}
...
\caption[Pulse height spectrum generated by the oscilloscope]{An oscilloscope based pulse height spectrum in the same scheme as the persistence figure fig(\ref{fig:persistence}). The distance between the peak maxima delivers an estimation of the gain of the SiPM at this bias-voltage. Image credit \cite{Initial_testing_at_MPIK_wb_July_18_2016}}
\label{fig:Osci_PHS}
\end{centering}
\end{figure}
